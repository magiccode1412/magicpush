<template>
  <div class="space-y-6 animate-fade-in">
    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">关于此项目</h2>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
      ...
      <div class="prose dark:prose-invert max-w-none">
        <p class="text-gray-600 dark:text-gray-400 leading-relaxed">
          MagicPush 是一个轻量级的消息推送平台，支持多种推送渠道，可以帮助您快速实现消息推送功能。
          通过统一的 API 接口，您可以轻松地向多个平台发送通知消息。
        </p>
      </div>
    </div>
    ...
    <!-- 支持的渠道 -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">支持的推送渠道</h3>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div
          v-for="channel in supportedChannels"
          :key="channel.name"
          class="flex items-center gap-3 p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50"
        >
          <div :class="['w-10 h-10 rounded-lg flex items-center justify-center', channel.bgColor]">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
          ...
          :key="feature.title"
          class="flex gap-3 p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50"
        >
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
      ...
      <div class="flex flex-wrap gap-3">
        <span
          ...
          :key="tech"
          class="px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300"
        >
          {{ tech }}
        </span>
      </div>
...
<script setup>
import { Bell, Github, Zap, Shield, Database, Globe, Code, Layers, MessageSquare, Send, Bot, Radio, MessageCircle, Link, Mail, Cat, Building2, Smartphone, FileText } from 'lucide-vue-next'
...
const supportedChannels = [
  ...
  { name: '元宝 Bot', icon: Cat, bgColor: 'bg-violet-100 dark:bg-violet-900/30', iconColor: 'text-violet-600 dark:text-violet-400' },
  ...
  { name: 'ShowDoc', icon: FileText, bgColor: 'bg-sky-100 dark:bg-sky-900/30', iconColor: 'text-sky-600 dark:text-sky-400' },
]

const features = [
  { title: '多渠道支持', description: '支持企业微信群机器人、Telegram、飞书、钉钉、SMTP邮件、Gotify、Bark、Meow、PushMe、息知、企业微信应用、元宝Bot、ntfy、PushDeer、iGot、群晖Chat、ShowDoc等多种推送渠道', icon: Layers },
